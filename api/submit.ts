...
import { rankPost, stored, markdownReport } from './_lib/relevance.js';
...
export async function POST(request: Request): Promise<Response> {
  ...
  try {
    ...
    // Rank only the post itself. The submitter's note never reaches the model.
    const relevance = post.text
      ? await rankPost({ text: post.text, source: post.source, authorName: post.author?.name, authorHandle: post.author?.handle })
      : null;
    if (relevance?.verdict === 'reject') {
      return json(422, { error: 'That post does not look like it shows a use of Jev, so it was not sent for review. If that is wrong, open an issue on the GitHub repository.' });
    }
    ...
    const tag = relevance ? ` [${relevance.verdict === 'publish' ? 'likely' : 'check'} ${relevance.score.toFixed(2)}]` : '';
    const pr = await openPullRequest(post.id, entry, `Community submission: ${post.id}${tag}`, body);
